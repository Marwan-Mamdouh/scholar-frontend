...
def format_job_message(job: Job) -> str:
    ...
    lines.append("")
    lines.append(f'🔗 <a href="{_escape_html(job.url)}">Apply Now</a>')
...
def send_job(job: Job, target_topics: list[str] | None = None) -> dict:
    """Route and send a job to matching topics. Returns {topic_key: success}."""
    ...
    for topic_key in target_topics:
        ...
        if thread_id is None:
            log.warning(f"  ✗ Topic not configured ({topic_key}) for: {job.title}")
