  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 ">
        <Accordion title="web app team" subtitle={`${team.length} members`} icon={CodeIcon}>
        ...
        <Accordion title="Academia team" subtitle={`${team.length} members`} icon={BookOutlineIcon}>
          <div className="space-y-4">
            {team.map((member) => (
              <TeamCard key={member.id} {...member}/>
            ))}
        ...
        <Accordion title="Industry Team" subtitle={`${team.length} members`} icon={IndustryIcon}>
          <div className="space-y-4">
            {team.map((member) => (
              <TeamCard key={member.id} {...member}/>
            ))}
