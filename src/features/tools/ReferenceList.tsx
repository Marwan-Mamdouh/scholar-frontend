    ...
    <ul className="flex flex-col gap-2.5">
      {links.map((link) => (
        <li key={link.href}>
          ...
          >
            <Github className={`h-5.5 w-5.5 shrink-0 ${theme.text}`} />

            <span className="min-w-0 flex-1">
              <span className="block font-main break-words text-neutral-50">
                {link.name}
              </span>
              <span className="mt-0.5 block font-secondary text-sm text-neutral-200">
                {link.desc}
              </span>
            </span>
